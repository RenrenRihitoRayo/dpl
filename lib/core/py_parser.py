...
# boring
def safe_run(code, frame=None, thread_event=IS_STILL_RUNNING):
    "Run code generated by 'process'"
    p = 0
    end_time = start_time = 0
    if isinstance(code, str):
    ...
    while p < len(code) and not IS_STILL_RUNNING.is_set():
        ...
        argc = len(args)
        types = *map(type, args),
        if ins == "fn" and argc >= 1 and types[0] == str:
            name, *params = args
            temp = get_block(code, p)
            if temp is None:
                break
            ...
            varproc.debug.update(args[0])
        elif ins == "get_time" and argc == 1 and types[0] == str:
            frame[-1][args[0]] = time.time()
        elif ins == "pub" and argc >= 2 and args[0] == "fn" and types[1] == str:
            _, name, *params = args
            temp = get_block(code, p)
            if temp is None:
                break
            ...
            varproc.rset(frame[-1], name, temp)
        elif ins == "export" and argc == 3 and args[0] == "set" and types[1] == str:
            _, name, value = args
            varproc.rset(frame[-1], "_export." + name, value)
            varproc.rset(frame[-1], name, value)
        elif ins == "for" and argc == 3 and args[1] == "in" and types[0] == str and types[2] in (str, list, tuple, dict):
            name, _, iter = args
            temp = get_block(code, p)
            if temp is None:
                break
                ...
                for i in iter:
                    ...
                    if err:
                        if err == error.STOP_RESULT:
                            ...
                            continue
                        return err
        elif ins == "enum" and argc == 1 and types[0] == str:
            name = args[0]
            names = set()
            temp = get_block(code, p)
            if temp is None:
                ...
                p, body = temp
            for lp, _, ins, _ in body:
                if type(ins) != str:
                    name = getattr(type(ins), "__name__", "unknown type")
                    erorr.error(lp, file, "Should be a string or an identifier, but your dumbass put in some how a{'n' if name[0] in 'aeiou' else ''} {name}!")
                    return error.TYPE_ERROR
                names.add(ins)
            tmp = frame[-1][name] = {}
            for n in names:
            ...
            pprint(args[0])
        elif ins == "loop" and argc == 1 and types[0] == int:
            temp = get_block(code, p)
            if temp is None:
                break
            else:
                ...
                while not thread_event.is_set():
                    ...
                    if err:
                        if err == error.STOP_RESULT:
                            ...
                            continue
                        return err
        elif ins == "dlopen" and argc == 2 and types == (str, str):
            if args[1].startswith("{") and args[1].endswith("}"):
                file = info.get_path_with_lib(args[1][1:-1])
            else:
                file = args[1]
            ...
            ext_s.dpl.ffi.dlclose(args[0])
        elif ins == "getc" and argc == 2 and types[0] == str:
            frame[-1][args[0]] = getattr(args[1], args[0], constants.none)
        elif ins == "cdef" and argc == 1 and types[0] == str:
            ext_s.dpl.ffi.cdef(args[0])
        elif ins == "stop" and argc == 0:
            return error.STOP_RESULT
        elif ins == "skip" and argc == 0:
            ...
            if (err := argproc.parse_match(frame, body, args[0])) > 0:
                return err
        elif ins == "exec" and argc == 3 and types == (str, str, dict):
            if err:=run(process(args[0] ,name=args[1]), frame=args[2]):
                return err
        elif ins == "sexec" and argc == 4 and types == (str, str, str, dict):
            error.silent()
            frame[-1][args[0]] = run(process(args[1], name=args[2]), frame=args[3])
            error.active()
        elif ins == "fallthrough" and argc == 0:
            ...
            if t := varproc.rset(frame[-1], args[0], args[1]):
                ...
                return error.NAME_ERROR
        elif ins == "const" and argc == 2 and types[0] == str:
            name = args[0]
            if varproc.rset(frame[-1], name, args[1]):
                error.error(
                    pos,
            ...
            if consts:
                ...
                frame[-1]["_const"] = [name]
        elif ins == "fset" and argc == 2 and types[0] == str:
            varproc.rset(frame[-1], args[0], args[1], meta=False)
            ...
            for name in args:
                ...
                if consts and name in consts:
                    consts.remove(name)
        elif ins == "module" and argc == 1 and types[0] == str:
            name = args[0]
            temp = [frame[-1]]
            varproc.nscope(temp)
            temp[-1]["_export"] = {}
            ...
            varproc.rset(frame[-1], args[0], objects.make_object(args[0]))
        elif ins == "new" and argc == 2 and types == (dict, str):
            obj = args[0]
            if obj == state.bstate("nil"):
                error.error(pos, file, f"Unknown object")
                break
            varproc.rset(obj, "_internal.instance_name", args[1])
            varproc.rset(frame[-1], args[1], copy(obj))
        elif ins == "method" and argc >= 2 and types[:2] == (dict, str):
            self, name, *params = args
            if self == state.bstate("nil"):
                error.error(
                    pos, file, "Cannot bind a method to a value that isnt a context!"
            ...
            ct, unit = utils.convert_sec(time.perf_counter() - start_time)
            error.info(f"Elapsed time: {args[0]} {ct:,.8f}{unit}")
        elif ins == "cmd" and argc == 1 and types[0] == str:
            os.system(args[0])
        elif ins == "pass":
            ...
        elif ins == "thread" and argc == 0:
            ...
            threads.append(th_obj)
            th_obj.start()
        elif ins == "new_thread_event" and argc == 1 and types[0] == str:
            frame[-1][args[0]] = (temp := threading.Event())
            thread_events.append(temp)
        elif ins == "thread" and argc == 1:
            if not isinstance(args[0], threading.Event):
                error.error(pos, file, "The given thread event was invalid!")
            ...
            my_exit()
        elif ins == "exit" and argc == 1 and typed[0] == int:
            my_exit()
        elif ins == "return":  # Return to the latched names
            if not (temp := varproc.rget(frame[-1], "_returns")) != state.bstate("nil"):
                ...
