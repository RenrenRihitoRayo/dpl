        ...
        if ins == "fn" and argc >= 1:
            ...
            try:
                ...
                if argc == 2 and isinstance(args[0], dict) and args[0].get("[KWARGS]"):
                    ...
                    res = ext_s.call_w_body(temp, frame, file, body, pa, args[0])
                else:
                    res = ext_s.call_w_body(temp, frame, file, body, args)
            ...
            try:
                if argc == 3 and isinstance(args[0], dict) and args[0].get("[KWARGS]"):
                    args[0].pop("[KWARGS]")
                    pa = args[0].pop("[PARGS]", tuple())
                    res = ext_s.call(temp, frame, file, pa, args[0])
                else:
                    res = ext_s.call(temp, frame, file, args)
                if isinstance(res, tuple):
                    if res is not None:
                        for name, value in zip(rets, res):
                            varproc.rset(frame[-1], name, value)
